...
# Configure logging
logging.basicConfig(filename='credit_card_default_prediction.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
# creating 4 Dicts which are used to convert to str to int before giving input to ML module
S_DICT = {'Male': 1, 'Female': 2}
M_DICT = {'Married': 1, 'Single': 2, 'Others': 3}
E_DICT = {'Graduate school': 1, 'University': 2, 'High school': 3, 'Others': 4}
PAY_DICT = {'Zero Bill': 0, 'Paid duly': -1, '1 Month Delay': 1, '2 Months Delay': 2, '3 Months Delay': 3,
            '4 Months Delay': 4, '5 Months Delay': 5, '6 Months Delay': 6, '7 Months Delay': 7, '8 Months Delay': 8,
            '9 Months & Above Delay': 9}
...
# Function which handles multi transactions
def multi_cust(file):
    if file:
        ...
        df['AVG_BILL_AMT'] = ((df['1st Month.1'] + df['2nd Month.1'] + df['3rd Month.1'] + df['4th Month.1'] + df[
            '5th Month.1'] + df['6th Month.1']) / 6)
        ...
        sl.download_button(label='Download table', data=data_frame, mime='text/csv',
                           file_name='Bill Payment Status for Next month')

# Function to print output which also converts numeric output from ML module to understandable STR
def pred_out(num):
    if num == 1:
        sl.warning('This customer IS LIKELY TO DEFAULT next month..!')
        logging.info('Prediction: LIKELY TO DEFAULT')
    else:
        sl.success('This customer IS NOT LIKELY TO DEFAULT next month..!')
        logging.info('Prediction: NOT LIKELY TO DEFAULT')

# Title
sl.title('Credit Card Default Prediction')

# Selection
rad_b = sl.radio('Please select whether you want to give data for a single customer', options=['Single'])
...
def main():
    if rad_b == 'Single':
        ...
        C_Limit = col2.number_input('Limit Amount in $')
        # PAY input
        L_opt = ['Zero Bill', 'Paid duly', '1 Month Delay', '2 Months Delay', '3 Months Delay', '4 Months Delay',
                 '5 Months Delay', '6 Months Delay', '7 Months Delay', '8 Months Delay', '9 Months & Above Delay']
        ...
        form.subheader('Amount paid for the previous bill in $')
        ...
        # Creating a new feature Average Bill Amount
        avg_bill_amt = np.mean(BAmt1 + BAmt2 + BAmt3 + BAmt4 + BAmt5 + BAmt6)
        features = [C_Limit, C_sex, C_edu, C_marg, C_age, pay1, pay2, pay3, pay4, pay5, pay6, BAmt1, BAmt2, BAmt3,
                    BAmt4, BAmt5, BAmt6, PAmt1, PAmt2, PAmt3, PAmt4, PAmt5, PAmt6, avg_bill_amt]
        ...
        # If the predict button is clicked, it will predict
        if P_status:
            pred_out(pred)
